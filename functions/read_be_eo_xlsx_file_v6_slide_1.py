"""
подготовка таблицы для слайда №1 - сводная диаграмма динамики изменения количества и возраста парка 
"""
    ...
    for status_condition, status_condition_rus in status_condition_dict.items():
      ...
      if status_condition == "on_balance":
        if sap_system_status not in sap_system_status_ban_list:
          ...
          # последний день года, в котором началась эксплуатация 
          time_operation_point_date = time_operation_point_date.replace(day=31, month = 12)
          while time_operation_point_date <= operation_finish_date:
            ...
            temp_dict['eo_count_on_balance'] = eo_code
            result_data_list.append(temp_dict)
            time_operation_point_date = time_operation_point_date + relativedelta(months=12)

  slide_1_df_temp = pd.DataFrame(result_data_list)

  list_of_be = [1100]
  for be_code in list_of_be:
    df = slide_1_df_temp.loc[slide_1_df_temp['be_code']==be_code]
    ...
    wb.save(f"temp_data/df_{be_code}_slide_1.xlsx")